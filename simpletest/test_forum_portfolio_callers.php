 ...
 * is due to change.
 */
Mock::generate('forumanonymous_portfolio_caller', 'mock_caller');
Mock::generate('portfolio_exporter', 'mock_exporter');

class testForumPortfolioCallers extends portfoliolib_test {
/*
    public static $includecoverage = array('lib/portfoliolib.php', 'mod/forumanonymous/lib.php');
    public $module_type = 'forumanonymous';
    ...
    public function setUp() {
        ...
        $settings = array('quiet' => 1,
                          ...
                          'discussions_per_forumanonymous' => 5,
        ...
        $discussions = $DB->get_records('forumanonymous_discussions', array('forumanonymous' => $first_module->id));
        $first_discussion = reset($discussions);

        $posts = $DB->get_records('forumanonymous_posts', array('discussion' => $first_discussion->id));
        $first_post = reset($posts);

        $callbackargs = array('postid' => $first_post->id, 'discussionid' => $first_discussion->id);
        $this->postcaller = parent::setup_caller('forumanonymous_portfolio_caller', $callbackargs, $first_post->userid);

        unset($callbackargs['postid']);
        $this->discussioncaller = parent::setup_caller('forumanonymous_portfolio_caller', $callbackargs, $first_post->userid);
