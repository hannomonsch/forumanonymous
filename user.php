...
$url = new moodle_url('/mod/forumanonymous/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
//add_to_log(($isspecificcourse)?$courseid:SITEID, "forumanonymous", "user report", 'user.php?'.$url->get_query_string(), $userid);
...
if ($isspecificcourse) {
    ...
    $courses = forumanonymous_get_courses_user_posted_in($user, $discussionsonly);
}

// Get the posts by the requested user that the current user can access.
$result = forumanonymous_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_forumanonymous');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->shortname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_forumanonymous');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'forumanonymous');
        } else {
            $notification = get_string('nopostsmadebyyou', 'forumanonymous');
        }
    } else if ($canviewuser) {
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'forumanonymous', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'forumanonymous', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'forumanonymous');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('forumanonymous_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular forumanonymous,
...
foreach ($result->posts as $post) {
    if (!isset($result->forumanonymouss[$post->forumanonymous]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $forumanonymous = $result->forumanonymouss[$post->forumanonymous];
    $cm = $forumanonymous->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $forumanonymousurl = new moodle_url('/mod/forumanonymous/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/forumanonymous/discuss.php', array('d' => $post->discussion));
    ...
    if ($forumanonymous->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($forumanonymous->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $forumanonymousurl;
        } else {
    ...
    $forumanonymousname = format_string($forumanonymous->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($forumanonymousurl, $forumanonymousname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $forumanonymousname);
    }
    if ($forumanonymous->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/forumanonymous/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "forumanonymous"));

    $postoutput[] = forumanonymous_print_post($post, $discussion, $forumanonymous, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_forumanonymous', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_forumanonymous', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        $pagetitle = get_string('discussionsstartedbyuserincourse', 'mod_forumanonymous', $a);
    } else {
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_forumanonymous', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'forumanonymous', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'forumanonymous'));
}
