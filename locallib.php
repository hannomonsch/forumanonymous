...
/**
 * Library of functions for forumanonymous outside of the core api
 */
 ...
 */
class forumanonymous_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'forumanonymous');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('forumanonymous_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'forumanonymous');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('forumanonymous_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'forumanonymous');
        }

        if (!$this->forumanonymous = $DB->get_record('forumanonymous', array('id' => $this->discussion->forumanonymous))) {
            throw new portfolio_caller_exception('invalidforumanonymousid', 'forumanonymous');
        }

        if (!$this->cm = get_coursemodule_from_instance('forumanonymous', $this->forumanonymous->id)) {
	...
	$fs = get_file_storage();
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumanonymous', 'attachment', $this->post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumanonymous', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = forumanonymous_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumanonymous', 'attachment', $post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumanonymous', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/forumanonymous/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/forumanonymous/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('forumanonymousdiscussion' . $this->discussionid,
                    get_string('discussion', 'forumanonymous') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single forumanonymous post,
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('forumanonymouspost' . $post->id,  $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id])) {
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'forumanonymouspost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in forumanonymous_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_forumanonymous', 'post', $post->id, $format);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'forumanonymous') . '</b>:<br /><br />';
	...
	$context = context_module::instance($this->cm->id);
        if ($this->post) {
            return (has_capability('mod/forumanonymous:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/forumanonymous:exportownpost', $context)));
        }
        return has_capability('mod/forumanonymous:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'forumanonymous');
    }
