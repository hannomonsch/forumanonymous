...
$forumanonymousid = trim(optional_param('forumanonymousid', 0, PARAM_INT));      // ForumID to search for
...
if (empty($search)) {   // Check the other parameters instead
    ...
    if (!empty($forumanonymousid)) {
        $search .= ' forumanonymousid:'.$forumanonymousid;
    }
    if (!empty($user)) {
        $search .= ' '.forumanonymous_clean_search_terms($user, 'user:');
    }
    if (!empty($subject)) {
        $search .= ' '.forumanonymous_clean_search_terms($subject, 'subject:');
    }
    if (!empty($fullwords)) {
        $search .= ' '.forumanonymous_clean_search_terms($fullwords, '+');
    }
    if (!empty($notwords)) {
        $search .= ' '.forumanonymous_clean_search_terms($notwords, '-');
    }
    if (!empty($phrase)) {
        $search .= ' "'.$phrase.'"';
    }
...
if ($search) {
    $search = forumanonymous_clean_search_terms($search);
}
...
//add_to_log($course->id, "forumanonymous", "search", "search.php?id=$course->id&amp;search=".urlencode($search), $search);

$strforumanonymouss = get_string("modulenameplural", "forumanonymous");
$strsearch = get_string("search", "forumanonymous");
$strsearchresults = get_string("searchresults", "forumanonymous");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strforumanonymouss, new moodle_url('/mod/forumanonymous/index.php', array('id'=>$course->id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'forumanonymous'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    forumanonymous_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchterms = str_replace('forumanonymousid:', 'instance:', $search);
$searchterms = explode(' ', $searchterms);

$searchform = forumanonymous_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/mod/forumanonymous/search.php', array('id'=>$course->id)));
$PAGE->navbar->add(s($search, true));
if (!$posts = forumanonymous_search_posts($searchterms, $course->id, $page*$perpage, $perpage, $totalcount)) {
    $PAGE->set_title($strsearchresults);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string("nopostscontaining", "forumanonymous", $search));
...
echo '<a href="search.php?id='.$course->id.
                         '&amp;user='.urlencode($user).
                         '&amp;userid='.$userid.
                         '&amp;forumanonymousid='.$forumanonymousid.
                         ...
                         '&amp;datefrom='.$datefrom.
                         '&amp;showform=1'.
                         '">'.get_string('advancedsearch','forumanonymous').'...</a>';
echo '</div>';
...
foreach ($posts as $post) {
    ...
    // Replace the simple subject with the three items forumanonymous name -> thread name -> subject
    ...
    $fullsubject = "<a href=\"view.php?f=$forumanonymous->id\">".format_string($forumanonymous->name,true)."</a>";
    ...
    //Unfortunately seem to have do this individually as posts may be from different forumanonymouss
    ...
    // the start of the message text. However, do not do the highlighting here. forumanonymous_print_post
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','forumanonymous');
        $post->message = '<p class="highlight2">'.$strmissingsearchterms.' '.$missing_terms.'</p>'.$post->message;
    }

    // Prepare a link to the post in context, to be displayed after the forumanonymous post.
    $fulllink = "<a href=\"discuss.php?d=$post->discussion#p$post->id\">".get_string("postincontext", "forumanonymous")."</a>";
 ...
 */
function forumanonymous_print_big_search_form($course) {
    global $CFG, $DB, $words, $subject, $phrase, $user, $userid, $fullwords, $notwords, $datefrom, $dateto, $PAGE, $OUTPUT;

    echo $OUTPUT->box(get_string('searchforumanonymousintro', 'forumanonymous'), 'searchbox boxaligncenter', 'intro');

    echo $OUTPUT->box_start('generalbox boxaligncenter');

    echo html_writer::script('', $CFG->wwwroot.'/mod/forumanonymous/forumanonymous.js');

    echo '<form id="searchform" action="search.php" method="get">';
    echo '<table cellpadding="10" class="searchbox" id="form">';

    echo '<tr>';
    echo '<td class="c0"><label for="words">'.get_string('searchwords', 'forumanonymous').'</label>';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="phrase">'.get_string('searchphrase', 'forumanonymous').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="phrase" id="phrase" value="'.s($phrase, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="notwords">'.get_string('searchnotwords', 'forumanonymous').'</label></td>';
    ...
    if ($DB->get_dbfamily() == 'mysql' || $DB->get_dbfamily() == 'postgres') {
        echo '<tr>';
        echo '<td class="c0"><label for="fullwords">'.get_string('searchfullwords', 'forumanonymous').'</label></td>';
        echo '<td class="c1"><input type="text" size="35" name="fullwords" id="fullwords" value="'.s($fullwords, true).'" alt="" /></td>';
        echo '</tr>';
    }

    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdatefrom', 'forumanonymous').'</td>';
    echo '<td class="c1">';
    if (empty($datefrom)) {
        $datefromchecked = '';
    ...
    echo '<input name="timefromrestrict" type="checkbox" value="1" alt="'.get_string('searchdatefrom', 'forumanonymous').'" onclick="return lockoptions(\'searchform\', \'timefromrestrict\', timefromitems)" '.  $datefromchecked . ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdateto', 'forumanonymous').'</td>';
    echo '<td class="c1">';
    if (empty($dateto)) {
        $datetochecked = '';
        $dateto = time()+3600;
    ...
    echo '<input name="timetorestrict" type="checkbox" value="1" alt="'.get_string('searchdateto', 'forumanonymous').'" onclick="return lockoptions(\'searchform\', \'timetorestrict\', timetoitems)" ' .$datetochecked. ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="menuforumanonymousid">'.get_string('searchwhichforumanonymouss', 'forumanonymous').'</label></td>';
    echo '<td class="c1">';
    echo html_writer::select(forumanonymous_menu_list($course), 'forumanonymousid', '', array(''=>get_string('allforumanonymouss', 'forumanonymous')));
    echo '</td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="subject">'.get_string('searchsubject', 'forumanonymous').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="subject" id="subject" value="'.s($subject, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="user">'.get_string('searchuser', 'forumanonymous').'</label></td>';
    ...
    echo '<input type="submit" value="'.get_string('searchforumanonymouss', 'forumanonymous').'" alt="" /></td>';
    echo '</tr>';
 ...
 */
function forumanonymous_clean_search_terms($words, $prefix='') {
 ...
 * @todo Document this function
 */
function forumanonymous_menu_list($course)  {
    ...
    foreach ($modinfo->instances['forumanonymous'] as $cm) {
        if (!$cm->uservisible) {
            continue;
        }
